...
jvm.loadClass = function (className) {
	...
	var clazz = jvm.classes[className];
	if (clazz) return clazz;

	function makePrimitiveClazz(descriptor, name) {
		var clz = {
			name: name,
	...
	switch (className) {
		case 'V': return this.classes['V'] = makePrimitiveClazz('V', 'void');
		case "Z": return this.classes['Z'] = makePrimitiveClazz('Z', 'boolean');
		case "C": return this.classes['C'] = makePrimitiveClazz('C', 'char');
		case "F": return this.classes['F'] = makePrimitiveClazz('F', 'float');
		case "D": return this.classes['D'] = makePrimitiveClazz('D', 'double');
		case "B": return this.classes['B'] = makePrimitiveClazz('B', 'byte');
		case "S": return this.classes['S'] = makePrimitiveClazz('S', 'short');
		case "I": return this.classes['I'] = makePrimitiveClazz('I', 'int');
		case "J": return this.classes['J'] = makePrimitiveClazz('J', 'long');
	}
	...
	if (className[0] == "[") {
		...
		return this.classes[className] = clazz;
	}

	clazz = jvm.loadClassFile(className);
	if (clazz.superClass) {
...
jvm.getClassObjectFromDescriptor = function (descriptor) {
	if (!(typeof descriptor === 'string')) debugger; //descriptor must be native string
	return jvm.getClassObject(jvm.loadClass(descriptor));
};

jvm.getClassObject = function (theClazz) {
	var key = theClazz.name;
	if (!jvm.classObjects[key]) {
		...
		result.setField("theClazz", theClazz); //TODO 放到metadata里
...
jvm.newArray = function (componentClazz, count) {
	...
	if (componentClazz == jvm.loadClass('I')) {
		for (var i = 0; i < count; i++) {
			arrObj[i] = 0;
		}
	}
	if (componentClazz == jvm.loadClass('J')) {
		for (var i = 0; i < count; i++) {
